...
def write_u8(mut buf: List[UInt8], pos: Int, val: UInt8):
    buf[pos] = val


def write_u16_le(mut buf: List[UInt8], pos: Int, val: UInt16):
    buf[pos]     = UInt8(val & UInt16(0xFF))
    buf[pos + 1] = UInt8((val >> 8) & UInt16(0xFF))


def write_u32_le(mut buf: List[UInt8], pos: Int, val: UInt32):
...
def write_i32_le(mut buf: List[UInt8], pos: Int, val: Int32):
    var v = Int64(val)
...
def write_i64_le(mut buf: List[UInt8], pos: Int, val: Int64):
...
def write_u64_le(mut buf: List[UInt8], pos: Int, val: UInt64):
...
def read_u8(buf: List[UInt8], pos: Int) raises -> UInt8:
    if pos < 0 or pos >= len(buf):
        raise Error("flatbuffers: read_u8 out of bounds at " + String(pos))
    return buf[pos]


def read_u16_le(buf: List[UInt8], pos: Int) raises -> UInt16:
...
def read_u32_le(buf: List[UInt8], pos: Int) raises -> UInt32:
...
def padding_to(pos: Int, alignment: Int) -> Int:
...
struct FieldLoc(Copyable, Movable):
    ...
    def __init__(out self, slot: Int, offset: UInt32):
        self.slot = slot
        self.offset = offset
...
struct FlatBufferBuilder(Movable):
    ...
    def prepend_u8(mut self, val: UInt8) raises:
        self._prep(1, 1)
        self._head -= 1
        self._buf[self._head] = val

    def prepend_bool(mut self, val: Bool) raises:
        self.prepend_u8(UInt8(1) if val else UInt8(0))

    def prepend_u16(mut self, val: UInt16) raises:
        self._prep(2, 2)
        self._head -= 2
        write_u16_le(self._buf, self._head, val)

    def prepend_i16(mut self, val: Int16) raises:
        self._prep(2, 2)
        self._head -= 2
        var v = Int32(val)
    ...
    def prepend_u32(mut self, val: UInt32) raises:
        self._prep(4, 4)
        self._head -= 4
        write_u32_le(self._buf, self._head, val)

    def prepend_i32(mut self, val: Int32) raises:
        self._prep(4, 4)
        self._head -= 4
        write_i32_le(self._buf, self._head, val)

    def prepend_u64(mut self, val: UInt64) raises:
        self._prep(8, 8)
        self._head -= 8
        write_u64_le(self._buf, self._head, val)

    def prepend_i64(mut self, val: Int64) raises:
        self._prep(8, 8)
        self._head -= 8
        write_i64_le(self._buf, self._head, val)

    def prepend_f32(mut self, val: Float32) raises:
        self._prep(4, 4)
        self._head -= 4
        write_f32_le(self._buf, self._head, val)

    def prepend_f64(mut self, val: Float64) raises:
        self._prep(8, 8)
        self._head -= 8
    ...
    def create_vector_u32(mut self, data: List[UInt32]) raises -> UInt32:
        var n = len(data)
        self._prep(4, n * 4)
        for i in range(n - 1, -1, -1):
            self._head -= 4
    ...
    def create_vector_i32(mut self, data: List[Int32]) raises -> UInt32:
        var n = len(data)
        self._prep(4, n * 4)
        for i in range(n - 1, -1, -1):
            self._head -= 4
    ...
    def start_table(mut self) raises:
        if self._in_table:
    ...
    def _add_field(mut self, slot: Int) raises:
    ...
    def add_field_offset(mut self, slot: Int, val: UInt32) raises:
    ...
    def add_field_i8(mut self, slot: Int, val: Int8) raises:
        self.prepend_u8(UInt8(val))  # bitwise safe for single byte
        self._add_field(slot)

    def add_field_u8(mut self, slot: Int, val: UInt8) raises:
        self.prepend_u8(val)
        self._add_field(slot)

    def add_field_i16(mut self, slot: Int, val: Int16) raises:
        self.prepend_i16(val)
        self._add_field(slot)

    def add_field_u16(mut self, slot: Int, val: UInt16) raises:
        self.prepend_u16(val)
        self._add_field(slot)

    def add_field_i32(mut self, slot: Int, val: Int32) raises:
        self.prepend_i32(val)
        self._add_field(slot)

    def add_field_u32(mut self, slot: Int, val: UInt32) raises:
        self.prepend_u32(val)
        self._add_field(slot)

    def add_field_i64(mut self, slot: Int, val: Int64) raises:
        self.prepend_i64(val)
        self._add_field(slot)

    def add_field_f32(mut self, slot: Int, val: Float32) raises:
        self.prepend_f32(val)
        self._add_field(slot)

    def add_field_f64(mut self, slot: Int, val: Float64) raises:
        self.prepend_f64(val)
        self._add_field(slot)

    def add_field_bool(mut self, slot: Int, val: Bool) raises:
        self.prepend_bool(val)
        self._add_field(slot)

    def end_table(mut self) raises -> UInt32:
...
struct FlatBuffersReader(Movable):
    ...
    def read_i8(self, tp: UInt32, slot: Int, default: Int8 = 0) raises -> Int8:
    ...
    def read_i16(self, tp: UInt32, slot: Int, default: Int16 = 0) raises -> Int16:
        var voff = self._field_voffset(tp, slot)
        if voff == 0:
            return default
    ...
    def read_i32(self, tp: UInt32, slot: Int, default: Int32 = 0) raises -> Int32:
        var voff = self._field_voffset(tp, slot)
        if voff == 0:
            return default
        return read_i32_le(self._buf, Int(tp) + Int(voff))

    def read_u32(self, tp: UInt32, slot: Int, default: UInt32 = 0) raises -> UInt32:
        var voff = self._field_voffset(tp, slot)
        if voff == 0:
            return default
        return read_u32_le(self._buf, Int(tp) + Int(voff))

    def read_i64(self, tp: UInt32, slot: Int, default: Int64 = 0) raises -> Int64:
        var voff = self._field_voffset(tp, slot)
        if voff == 0:
            return default
        return read_i64_le(self._buf, Int(tp) + Int(voff))

    def read_f32(self, tp: UInt32, slot: Int, default: Float32 = 0.0) raises -> Float32:
        var voff = self._field_voffset(tp, slot)
        if voff == 0:
            return default
        return read_f32_le(self._buf, Int(tp) + Int(voff))

    def read_f64(self, tp: UInt32, slot: Int, default: Float64 = 0.0) raises -> Float64:
        var voff = self._field_voffset(tp, slot)
        if voff == 0:
            return default
        return read_f64_le(self._buf, Int(tp) + Int(voff))

    def read_bool(self, tp: UInt32, slot: Int, default: Bool = False) raises -> Bool:
        var voff = self._field_voffset(tp, slot)
        if voff == 0:
            return default
    ...
    def read_string(self, tp: UInt32, slot: Int) raises -> String:
        var voff = self._field_voffset(tp, slot)
        if voff == 0:
    ...
    def read_offset(self, tp: UInt32, slot: Int) raises -> UInt32:
        var voff = self._field_voffset(tp, slot)
        if voff == 0:
    ...
    def vec_u8(self, vec_pos: UInt32, i: UInt32) raises -> UInt8:
    ...
    def vec_u32(self, vec_pos: UInt32, i: UInt32) raises -> UInt32:
        var vlen = self.vector_len(vec_pos)
        if i >= vlen:
            raise Error("flatbuffers: vec index " + String(i) + " >= len " + String(vlen))
        return read_u32_le(self._buf, Int(vec_pos) + 4 + Int(i) * 4)

    def vec_i32(self, vec_pos: UInt32, i: UInt32) raises -> Int32:
        var vlen = self.vector_len(vec_pos)
        if i >= vlen:
            raise Error("flatbuffers: vec index " + String(i) + " >= len " + String(vlen))
        return read_i32_le(self._buf, Int(vec_pos) + 4 + Int(i) * 4)

    def vec_f32(self, vec_pos: UInt32, i: UInt32) raises -> Float32:
        var vlen = self.vector_len(vec_pos)
        if i >= vlen:
            raise Error("flatbuffers: vec index " + String(i) + " >= len " + String(vlen))
        return read_f32_le(self._buf, Int(vec_pos) + 4 + Int(i) * 4)

    def vec_f64(self, vec_pos: UInt32, i: UInt32) raises -> Float64:
        var vlen = self.vector_len(vec_pos)
        if i >= vlen:
            raise Error("flatbuffers: vec index " + String(i) + " >= len " + String(vlen))
    ...
    def read_string_at(self, str_pos: UInt32) raises -> String:
    ...
    def vec_string(self, vec_pos: UInt32, i: UInt32) raises -> String:
    ...
    def union_type(self, tp: UInt32, type_slot: Int) raises -> UInt8:
        return self.read_u8(tp, type_slot)

    def union_table(self, tp: UInt32, value_slot: Int) raises -> UInt32:
